...
Atomic Codex skills, one per video-editing operation. Each skill lives at `.Codex/skills/<name>/SKILL.md` and is independently invocable. Skills chain into the full shorts pipeline but are NOT a monolithic codebase.

**Stack:** whisper.cpp (local transcription) + Codex (semantic decisions, via host session or `/crew` tmux members — no API key) + ffmpeg (all media ops). Rails + Sidekiq is an optional wrapper, not required.

**No Anthropic API key.** Codex-driven skills (`pick-segments`, `segment-topics`, `verify-coherence`, `bookend-trim`, `trim-filler`, `chunk-captions`, `generate-title`, `pick-mood`) run inside the host Codex session or are dispatched to `/crew` members on tmux. All use the user's Codex subscription.
...
source video
  ...
  ├─ segment-topics   (Codex → topics.json)
  ├─ pick-segments    (Codex over transcript + RMS + topics → segments.raw.json, N spans)
  ├─ verify-coherence (tighten incoherent spans to the dominant topic)
  ├─ bookend-trim     (Codex snaps each span's [t0,t1] to sentence boundaries)
  └─ per span:
       cut-clip
        → rebase
        → trim-filler + cut-filler     (Codex cuts filler / trail-offs / digressive asides)
        ...
        → pick-mood + bg-music         (Codex picks ./songs/<mood>/ from clip transcript; bed at -18dB, last 5 picks blacklisted via ./songs/.recent)
...
- **No AGENTS.md per skill.** A single SKILL.md with frontmatter is the contract.
